...
export const KB: KbEntry[] = [
  ...
  },
  {
    ...
    content: [
      ...
      '',
      '**Passo a passo:** escolha o **produto** (a foto dele é a base do vídeo — a fonte pode ser o **Catálogo** ou o **TikTok Shop**, nas abas do seletor) → escolha **como gerar** (Animar a foto real OU Cena por IA com o produto) → escolha o **estilo** (360°, Cinemagraph, Loop, Unboxing, UGC… — os marcados "experimental" dependem de pessoa/áudio e a qualidade varia) → a **estrutura do roteiro** (Dor+Solução+Benefício, AIDA, PAS, ou os novos **🔥 Viral (10 momentos)** — estrutura psicológica que prende e converte — e **🎙️ Voz neutra** — roteiro para narração/avatar faceless de ~90s com checagem de fatos) → duração → **Gerar Reel**.',
      '',
  ...
  },
  {
    routes: ['/social/tendencias', '/social'],
    category: 'SOCIAL',
    title: 'Crescimento orgânico: perfis de referência + padrões vencedores',
    content: [
      'O Radar tem um **método de crescimento orgânico** em 4 pilares. A ideia central: **o algoritmo recompensa performance, não originalidade** — então não reinvente, modele o que já está bombando no seu nicho e adapte à sua marca.',
      '',
      '**1) Perfis de referência (Inteligência Global):** na seção **Perfis de referência** cole de 20 a 100 perfis do seu nicho — do mundo todo (EUA, Europa, Ásia). Pode colar **URLs** (instagram.com/perfil, tiktok.com/@perfil) ou **@handles**, vários por linha de uma vez. Marque o nicho e o país. Quanto mais perfis validados (com muitos seguidores e vídeos que estouram), melhor. Ser pioneiro de um formato que bomba lá fora é vantagem competitiva no Brasil.',
      '',
      '**2) Minerar perfis:** o botão **Minerar perfis** (topo) puxa via Apify os **posts recentes** desses perfis (últimos 7 dias por padrão). Eles entram em "O que está bombando" marcados como vindos de perfil. Funciona pra **Instagram e TikTok** hoje (X/YouTube entram depois). É uma chamada paga (centavos) — a coleta automática tem trava de 7 dias por perfil.',
      '',
      '**3) Analisar (Engenharia Reversa):** o botão **Analisar** manda a IA **decompor os posts vencedores** em **padrões**: o tipo de gancho, o formato, a estrutura narrativa, o CTA, o cenário e **por que aquilo performou**. Os padrões aparecem na seção **Padrões vencedores**.',
      '',
      '**4) O roteirista usa os padrões automaticamente:** quando você gera campanhas (Autopilot) ou Reels, a IA já recebe esses padrões vencedores como modelo ("modele a estrutura, não copie o conteúdo"). Você pode desligar isso na receita da campanha (campo *use_trend_patterns*).',
      '',
      '**Fluxo recomendado:** cole perfis → **Minerar perfis** → **Analisar** → gere campanhas/Reels normalmente. Para fechar o ciclo, escolha no roteiro o framework **🔥 Viral (10 momentos)** ou **🎙️ Voz neutra** (faceless/avatar).',
    ].join('\n'),
    tags: ['social', 'organico', 'crescimento', 'perfis', 'concorrentes', 'minerar', 'engenharia reversa', 'padroes', 'viral', 'apify', 'tendencias'],
  },
  {
