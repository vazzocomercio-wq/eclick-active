...
const RISK_OPTIONS: Array<{ value: DealRisk | null; label: string; color: string }> = [
  { value: null, label: 'Todos', color: 'bg-muted text-muted-foreground' },
  { value: 'low', label: 'Baixo', color: 'bg-emerald-500/15 text-emerald-400' },
  { value: 'medium', label: 'Médio', color: 'bg-yellow-500/15 text-yellow-400' },
  { value: 'high', label: 'Alto', color: 'bg-orange-500/15 text-orange-400' },
  { value: 'critical', label: 'Crítico', color: 'bg-red-500/15 text-red-400' },
];
    ...
    <div className="flex flex-col gap-2 border-b border-border bg-background/50 px-6 py-3">
      ...
      {expanded && (
        <div className="grid grid-cols-1 gap-3 rounded-lg border border-border bg-card/50 p-3 sm:grid-cols-3">
          <FilterField label="Tags (separadas por vírgula)">
            <Input
              value={tagsInput}
              onChange={(e) => setTagsInput(e.target.value)}
              onBlur={applyTextFilters}
              onKeyDown={(e) => e.key === 'Enter' && applyTextFilters()}
              placeholder="b2b, enterprise"
