    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="bg-background border border-border rounded-xl w-full max-w-2xl max-h-[88vh] flex flex-col overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-border flex items-center justify-between">
          ...
          <button onClick={onClose} aria-label="Fechar" className="text-muted-foreground hover:text-foreground p-1">
            <X size={18} />
          </button>
        </div>
                    ...
                    <button type="button" onClick={() => setPicked(p)}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {p.sku ? `SKU ${p.sku} · ` : ''}{p.in_stock ? `${p.stock} em estoque` : 'Sem estoque'}
                        </p>
                      </div>
        ...
        {picked && (
          <div className="px-5 py-3 border-t border-border space-y-2 bg-muted/20">
            <p className="text-xs text-muted-foreground">Observação (opcional)</p>
            <textarea value={extra} onChange={e => setExtra(e.target.value)} rows={2}
              placeholder="Ex: temos esse modelo em estoque pra entrega imediata."
