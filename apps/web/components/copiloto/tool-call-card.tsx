...
const TOOL_VISUAL: Record<ToolCallRecord['tool'], { icon: LucideIcon; color: string }> = {
  ...
  search_products: { icon: ShoppingBag, color: 'text-cyan-400' },
  get_product_details: { icon: ShoppingBag, color: 'text-cyan-400' },
  manage_cart: { icon: ShoppingCart, color: 'text-emerald-400' },
  apply_coupon: { icon: Tag, color: 'text-amber-400' },
  checkout: { icon: CreditCard, color: 'text-emerald-500' },
  check_whatsapp_order: { icon: Receipt, color: 'text-blue-400' },
  recommend_products: { icon: Lightbulb, color: 'text-yellow-400' },
};
